<script lang="ts">
  ...
  import { getBase } from "../../api/client.js";
  ...
  <div class="header-right">
    ...
    <button
      ...
      onclick={() => {
        ...
        for (const [k, v] of Object.entries(router.params)) {
          if (k !== "story" && k !== "desktop" && v) params.set(k, v);
        }
        const qs = params.toString();
        const appPath = qs ? `${route}?${qs}` : route;
        window.open(`${base}/story?path=${encodeURIComponent(appPath)}`, "_blank");
      }}
      title="Story mode"
      aria-label="Open in story mode"
    >
      <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor">
        <path d="M4 1a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V3a2 2 0 00-2-2H4zm0 1h8a1 1 0 011 1v10a1 1 0 01-1 1H4a1 1 0 01-1-1V3a1 1 0 011-1zm1.5 10a.5.5 0 100 1h5a.5.5 0 100-1h-5z"/>
      </svg>
    </button>
